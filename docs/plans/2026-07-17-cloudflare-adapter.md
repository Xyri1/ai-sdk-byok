...
**Goal:** Ship `@ai-sdk-byok/cloudflare` — a D1-backed `ByokStorageAdapter` plus a Workers-KV-backed `CredentialRecordCache`, both always-encrypted with AES-256-GCM sealed credentials — together with a runnable Workers example under `examples/cloudflare-worker` carrying a workerd end-to-end suite.
...
Mark the delivered docs items `- [x]`; the example item stays unchecked until Task 8 delivers it.
...
### Task 8: Runnable example `examples/cloudflare-worker` + workerd end-to-end tests

**Files:**
- Modify: `package.json` (root — add workspace)
- Create: `examples/cloudflare-worker/package.json`
- Create: `examples/cloudflare-worker/wrangler.jsonc`
- Create: `examples/cloudflare-worker/tsconfig.json`
- Create: `examples/cloudflare-worker/.dev.vars.example`
- Create: `examples/cloudflare-worker/.gitignore`
- Create: `examples/cloudflare-worker/migrations/0001_ai_sdk_byok_init.sql` (copy from the package)
- Create: `examples/cloudflare-worker/src/index.ts`
- Create: `examples/cloudflare-worker/public/index.html`
- Create: `examples/cloudflare-worker/vitest.config.ts`
- Create: `examples/cloudflare-worker/test/apply-migrations.ts`
- Create: `examples/cloudflare-worker/README.md`
- Modify: `README.md`, `README.zh-CN.md`, `AGENTS.md`, `specs/003-cloudflare-adapter/tasks.md`
- Test: `examples/cloudflare-worker/test/e2e.test.ts`

**Interfaces:**
- Consumes: the published-shape API from Tasks 4–5 (`d1Adapter`, `kvCredentialCache` from `@ai-sdk-byok/cloudflare`; `createByokManager`, `cachedStorage`, `AiSdkByokValidationError` from `ai-sdk-byok`) via npm workspace links; the migration file from Task 3.
- Produces: `Env` interface exported from `src/index.ts` (`{ DB: D1Database; BYOK_CACHE: KVNamespace; BYOK_MASTER_KEY: string }`); Hono app default export whose `fetch(request, env, ctx)` the tests drive.

**API-drift note:** the `cloudflare:test` / `cloudflareTest` plugin surface moves faster than the rest of this plan. The code below follows the vitest 4 integration as documented in July 2026. If a step fails on an import or config shape, consult the current `@cloudflare/vitest-pool-workers` docs (ctx7: `/cloudflare/cloudflare-docs`, topic "vitest integration") and adapt the shape — not the assertions.

- [ ] **Step 1: Register the workspace and scaffold config files**
...
```json
{
  "name": "ai-sdk-byok-cloudflare-worker-example",
  "private": true,
  "type": "module",
  "scripts": {
...
```jsonc
{
  ...
  // Placeholder ids work for local dev (miniflare); replace with real ids to deploy.
  "d1_databases": [
    {
      "binding": "DB",
      "database_name": "ai-sdk-byok-example",
      "database_id": "00000000-0000-0000-0000-000000000000"
    }
  ],
  "kv_namespaces": [{ "binding": "BYOK_CACHE", "id": "byok-cache-local" }]
}
```
...
`examples/cloudflare-worker/.dev.vars.example`:
...
Copy the migration and install:

```bash
mkdir -p examples/cloudflare-worker/migrations
cp packages/cloudflare/migrations/0001_ai_sdk_byok_init.sql examples/cloudflare-worker/migrations/
npm install
```

Expected: workspace links `ai-sdk-byok` and `@ai-sdk-byok/cloudflare` from the local packages (verify with `ls -l node_modules/@ai-sdk-byok`).

- [ ] **Step 2: Write the failing end-to-end tests**

`examples/cloudflare-worker/test/apply-migrations.ts`:
...
`examples/cloudflare-worker/vitest.config.ts`:
...
`examples/cloudflare-worker/test/e2e.test.ts`:
  ...
  it('chat uses the stored key against the provider', async () => {
    ...
    fetchMock
      ...
      .reply(
        200,
        (opts) => {
          capturedAuthorization = headerValue(opts.headers, 'authorization');
...
Run: `npm run test -w examples/cloudflare-worker`
Expected: PASS (6 tests) inside real workerd. If the OpenAI mock fails AI SDK response validation, read the SDK error and extend the mocked JSON body with the missing fields — do not loosen the `capturedAuthorization` or metadata-only assertions.

Also run: `npm run typecheck -w examples/cloudflare-worker`
Expected: clean.
...
# edit .dev.vars: BYOK_MASTER_KEY="$(openssl rand -base64 32)"
npm run dev -w examples/cloudflare-worker
```

Open the printed localhost URL: save a key (any `sk-…` string), see it listed with hint only, delete it. Chat requires a real OpenAI key. Ctrl-C when done.

- [ ] **Step 8: Write `examples/cloudflare-worker/README.md`**

```markdown
# ai-sdk-byok — Cloudflare Worker example

Runnable end-to-end example of `@ai-sdk-byok/cloudflare`: a Hono Worker with a key-management UI and a chat endpoint that constructs an AI SDK provider from the user's stored key, entirely server-side.
...
`wrangler dev` applies the D1 migration locally and serves the UI. Save a key, watch the list stay metadata-only, and chat using your own OpenAI key.

## Tests

`npm run test -w examples/cloudflare-worker` runs the end-to-end suite inside real workerd (`@cloudflare/vitest-pool-workers`): local D1/KV bindings, migrations applied in setup, and the outbound OpenAI call intercepted to prove the saved key is the one that reaches the provider.
...
- [ ] **Step 9: Update repo docs**

In root `README.md`, extend the Cloudflare section added in Task 7 with:
...
In `README.zh-CN.md`, add the equivalent line:

```markdown
可运行示例位于 [`examples/cloudflare-worker`](examples/cloudflare-worker) —— 密钥管理界面加服务端 Provider 构建，并附带在 workerd 中运行的端到端测试套件。
```

In `AGENTS.md` **Repository Layout**, after the `examples/nextjs-supabase` bullet, add:
...
In `specs/003-cloudflare-adapter/tasks.md`, mark the example item `- [x]`.

- [ ] **Step 10: Final verification and commit**

Run: `npm run check && npm run lint && npm run test -w examples/cloudflare-worker`
Expected: root typecheck/tests/build untouched by the example (it has its own tsconfig and vitest config) and the e2e suite green.

```bash
git add examples/cloudflare-worker package.json package-lock.json README.md README.zh-CN.md AGENTS.md specs/003-cloudflare-adapter/tasks.md
git commit -m "feat(examples): add runnable cloudflare worker example with workerd e2e"
```
...
- **Spec coverage:** requirements (adapter contract, sealed format, AAD, TTL layers, hashed KV keys, idempotent delete, ordering, fail-fast key validation, error hygiene) map to Tasks 2–5; security invariants to Tasks 2, 4, 5, 6, 8; docs/spec impact to Tasks 1, 7, 8; capacity note to Task 7 README; runnable example + workerd end-to-end to Task 8.
- **Deliberately out of plan (per spec):** sharded adapter, key-rotation tooling.
- **Type consistency:** `EncryptionKeyInput`, `CredentialSealer`, `createSealer`, `credentialAad` defined once in Task 2 and consumed by name in Tasks 4–5; `FakeD1Database`/`createFakeD1` defined in Task 3 and consumed in Tasks 4, 6; `createFakeKv` defined in Task 5 and consumed in Task 6; `Env` exported from the example worker in Task 8 and consumed by its tests.
